...
class SentenceGleuScorer(Scorer):
    """
    ...
    def set_reference(self, source_tokens, reference_tokens):
        """
        Sets the reference against hypotheses are scored.
        """
        self._reference = SentenceGleuReference(
            source_tokens,
            reference_tokens,
            self._arguments['n']
        )

class SentenceGleuReference(Reference):
    """
    Smoothed sentence-level GLEU as proposed by 
    Napoles, Sakaguchi, Post, and Tetreault (2015). 
    """

    def __init__(self, source_tokens, reference_tokens, n=4):
        """
        ...
        """
        Reference.__init__(self, source_tokens, reference_tokens)
        self.n = n
        ...
        self._source_length = len(self._source_tokens)
        self._source_ngrams = self._get_ngrams(self._source_tokens, self.n)
    ...
    def score(self, hypothesis_tokens):
        ...
        def ngram_precisions(src_ngrams, ref_ngrams, hyp_ngrams):
            ...
            for n in range(1, self.n+1):
                ...
                for src_ngram, src_ngram_count in src_ngrams[n-1].iteritems():
        ...
        np = ngram_precisions(self._source_ngrams, self._reference_ngrams, hypothesis_ngrams)
